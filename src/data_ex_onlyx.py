...
class H_sys:
    def __init__(self,V00,V01,V11,J01,J11,t,w0,w,single_x=True):
        ...
        w=w 
       # w0=2*w0
        matrix_Extend_onlyx= np.array([
        [0,-1j*w0,-1j*w0,0 ],
        [1j*w0,w-V00 ,0,-1j*w0],
        [1j*w0,0,-w+V00,-1j*w0],
        [0,1j*w0,1j*w0,0]])#+V00/2*np.eye(4)
        self.Extend=linalg.eig(matrix_Extend_onlyx)

        matrix_Extend_onlyx_2= np.array([
        [   -V00 +w ,-1j*w0,-1j*w0,0 ],
        [1j*w0,  2*w,0,-1j*w0],
        [1j*w0,0,0,-1j*w0],
        [0,1j*w0,1j*w0, -V00+w]])#+V00/2*np.eye(4)
        self.Extend2=linalg.eig(matrix_Extend_onlyx_2)

        matrix_Extend_onlyx_3= np.array([
        [  V00-w ,-1j*w0,-1j*w0,0 ],
        [1j*w0,0,0,-1j*w0],
        [1j*w0,0,  2*V00-2*w,-1j*w0],
        [0,1j*w0,1j*w0,   V00-w]])#+V00/2*np.eye(4)
        self.Extend3=linalg.eig(matrix_Extend_onlyx_3)

        matrix_Extend_onlyx_4= np.array([
        [0  ,-1j*w0,-1j*w0,0 ],
        [1j*w0, w+V00,0,-1j*w0],
        [1j*w0,0,  V00-w,-1j*w0],
        [0,1j*w0,1j*w0,0]])#+V00/2*np.eye(4)
        self.Extend4=linalg.eig(matrix_Extend_onlyx_4)

        


        I_block=-1j*w0*np.eye(4, dtype=float)
        I_block_c=1j*w0*np.eye(4, dtype=float)
        zero_block=np.zeros([4,4])
 
        row_1=np.concatenate((matrix_Extend_onlyx, I_block), axis=1)
        row_1=np.concatenate((row_1, I_block), axis=1)
        row_1=np.concatenate((row_1, zero_block), axis=1)
        
        row_2=np.concatenate((I_block_c,matrix_Extend_onlyx_2), axis=1)
        row_2=np.concatenate((row_2, zero_block), axis=1)
        row_2=np.concatenate((row_2, I_block), axis=1)
        
        row_3=np.concatenate((I_block_c,zero_block), axis=1)
        row_3=np.concatenate((row_3, matrix_Extend_onlyx_3), axis=1)
        row_3=np.concatenate((row_3, I_block), axis=1)
        
        row_4=np.concatenate((zero_block,I_block_c), axis=1)
        row_4=np.concatenate((row_4, I_block_c ), axis=1)
        row_4=np.concatenate((row_4, matrix_Extend_onlyx_4), axis=1)

        full_matrix_ex=np.concatenate((row_1,row_2), axis=0)
        full_matrix_ex=np.concatenate((full_matrix_ex,row_3), axis=0)
        full_matrix_ex=np.concatenate((full_matrix_ex,row_4), axis=0)
        print(full_matrix_ex.shape)
        self.Val_full=linalg.eig(full_matrix_ex)
    ...
    def get_extend(self):
        ...
        Ext_2_val=[]
        for i in range(0,len(Extend_val)):
            for j in range(0,len(Extend_val)):
                Ext_2_val.append(Extend_val[i]+Extend_val[j])
                Ext_2_val.append(Extend_val[i]-Extend_val[j])
        Extend_val_2=np.unique(np.abs(Ext_2_val))
        Extend_vec=np.round(self.normalize_all_eigen_vec(self.Extend[1].T[inde_arry]),5)
        print('extended1')
        print(Extend_val)
      #  print(Extend_val_2)

        inde_arry=np.argsort(self.Extend2[0])
        Extend_val=np.round((self.Extend2[0][inde_arry]),5)
        Ext_2_val=[]
        for i in range(0,len(Extend_val)):
            for j in range(0,len(Extend_val)):
                Ext_2_val.append(Extend_val[i]+Extend_val[j])
     ...
     #   print(Extend_val_2)
        ...
        inde_arry=np.argsort(self.Extend3[0])
        Extend_val=np.round((self.Extend3[0][inde_arry]),5)
        Ext_2_val=[]
        for i in range(0,len(Extend_val)):
            for j in range(0,len(Extend_val)):
                Ext_2_val.append(Extend_val[i]+Extend_val[j])
      ...
      #  print(Extend_val_2)
        ...
        inde_arry=np.argsort(self.Extend4[0])
        Extend_val=np.round((self.Extend4[0][inde_arry]),5)
        Ext_2_val=[]
        for i in range(0,len(Extend_val)):
            for j in range(0,len(Extend_val)):
                Ext_2_val.append(Extend_val[i]+Extend_val[j])
        ...
        print(Extend_val)
        print('-----full exciton--')
        print(np.unique(np.round(self.Val_full[0],5)))
        #print(Extend_val_2)
        ...
        E2_vec=np.round(self.normalize_all_eigen_vec(self.Val_1[1].T[inde_arry_E2]),5)
       # print(np.round(E2_vec.T,3))
        print( np.round(E2_val-self.Val_1[0][np.argmin((np.abs(self.Val_1[0])))],5))
 
        print('3_E')
        inde_arry_E3=np.argsort(self.Val_2[0])
        E3_val=np.round((self.Val_2[0][inde_arry_E3]),5)
        E3_vec=np.round(self.normalize_all_eigen_vec(self.Val_2[1].T[inde_arry_E3]),5)
        #print(np.round(self.Val_1[0],3))
        print(np.round(E3_val-self.Val_2[0][np.argmin((np.abs(self.Val_2[0])))],5))
